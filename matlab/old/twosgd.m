...
if strcmp(dname,'brat14')
	Ytrain = Ytrain - 1;
	Ytest= Ytest -1;
end
if strcmp(dname,'sphere4d2')
	Ytrain = Ytrain - 1;
	Ytest= Ytest -1;
end
...
% this code is to compute the kernel bandwidth (the so-called
% "median trick")
rand('seed', 1);
disp(['Sigma: ',num2str(sigma)]);
s = 1./(2*sigma^2);
% s = 1/bandwidth^2 => bandwidth = median trick;

reg_param = lambda;  % no regularization
...
n = 2^20  % overall rank, but the training is done one at a time.
blocksz = rank/2
batch_size = rank *10
